...
import { JobCardData, PublicJob } from '~/types/jobs'; // Import interfaces

// Create an animated version of Swipeable
...
export default function ListScreen() {
    ...
    const checkAnimations = useRef<{ [key: string]: RNAnimated.Value }>({});
    ...
    useEffect(() => {
        tabs.forEach(tab => {
            if (!checkAnimations.current[tab.id]) {
                checkAnimations.current[tab.id] = new RNAnimated.Value(
                    tab.id === 'flexible' ? 1 : 0,
                );
            }
        });
    ...
    const loadPageData = useCallback(
        async (page: number, isRefreshOperation: boolean = false) => {
            ...
            try {
                ...
                setJobs(prevJobs => {
                    const updatedJobs =
                        isRefreshOperation || page === 0
                            ? newItems
                            : [...prevJobs, ...newItems];
                    setCanLoadMore(
                        newItems.length === PAGE_SIZE &&
                            updatedJobs.length < data.total,
                    );
                    return updatedJobs;
                });
    ...
    useEffect(() => {
        tabs.forEach(tab => {
            ...
            RNAnimated.timing(checkAnimations.current[tab.id], {
                toValue: targetValue,
                duration: 200,
    ...
    const onDismiss = (jobId: string) => {
        setJobs((prevJobs) =>
            prevJobs.filter((job) => job.workAssignmentId !== jobId),
        );
        // Close the swipeable item if it's still open
        // if (swipeableRefs.current[jobId]) {
        //   swipeableRefs.current[jobId]?.close(); // Removed this line
        // }
    };
    ...
    const renderRightActions = (
        ...
        return (
            <RNAnimated.View style={{ transform: [{ translateX: trans }] }}>
                <RNPressable
                    onPress={() => onDismiss(jobId)}
                    className="bg-red-500 justify-center items-center w-20 h-full">
                    <Trash2 size={24} color="white" />
                    <Text className="text-white text-xs mt-1">Dismiss</Text>
                </RNPressable>
            </RNAnimated.View>
        );
    };
        ...
        return (
            <AnimatedSwipeable
                ref={(ref: Swipeable | null) => {
                    swipeableRefs.current[item.workAssignmentId] = ref;
                }}
                renderRightActions={(progress, dragX) =>
                    renderRightActions(progress, dragX, item.workAssignmentId)
                }
                        ...
                        {tabs.map(tab => (
                            <RNPressable
                                ...
                                <View className="flex-row items-center justify-center">
                                    ...
                                    <Text className="text-sm font-semibold text-secondary-foreground mr-4 opacity-50">
                                        {tab.label}
                                    </Text>
                                </View>
                            </RNPressable>
            ...
            <View
                        ...
                        {tabs.map(tab => (
                            <RNPressable
                                ...
                                <View className="flex-row items-center justify-center">
                                    ...
                                    <Text className="text-sm font-semibold text-secondary-foreground mr-4">
                                        {tab.label}
                                    </Text>
                                </View>
                            </RNPressable>
                ...
                <Animated.FlatList
                    ...
                    refreshControl={
                        <RefreshControl
                            refreshing={
                                loading && currentPage === 0 && jobs.length > 0
                            }
                            onRefresh={onRefresh}
                        />
                    }
